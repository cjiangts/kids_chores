#!/usr/bin/env python3
"""One-time migration: add generator multichoice-only flag to shared and kid DBs in a full backup zip."""
...
def migrate_shared_db(db_path: Path) -> bool:
    conn = duckdb.connect(str(db_path))
    try:
        if table_has_column(conn, "deck_generator_definition", "is_multichoice_only"):
            return False
        conn.execute("BEGIN TRANSACTION")
        try:
            conn.execute(
                """
                ALTER TABLE deck_generator_definition
                ADD COLUMN is_multichoice_only BOOLEAN DEFAULT FALSE
                """
            )
            conn.execute(
                """
...
def migrate_kid_db(db_path: Path) -> bool:
    conn = duckdb.connect(str(db_path))
    try:
        if table_has_column(conn, "cards", "is_multichoice_only"):
            return False
        conn.execute("BEGIN TRANSACTION")
        try:
            conn.execute(
...
def migrate_full_backup_zip(input_zip: Path, output_zip: Path) -> bool:
    ...
    with zipfile.ZipFile(input_zip, "r") as zin, zipfile.ZipFile(output_zip, "w", zipfile.ZIP_DEFLATED) as zout:
        for info in zin.infolist():
            ...
            data = zin.read(name)
            if not is_shared_db_zip_member(name) and not is_kid_db_zip_member(name):
                zout.writestr(info, data)
                continue

            with tempfile.TemporaryDirectory(prefix="type4_multichoice_zip_mig_") as tmp_dir:
                tmp_db = Path(tmp_dir) / Path(name).name
                tmp_db.write_bytes(data)
                if is_shared_db_zip_member(name):
                    migrated = migrate_shared_db(tmp_db) or migrated
                else:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Add is_multichoice_only to shared generator definitions and kid cards inside a full backup zip."
    )
    parser.add_argument(
        "--input-zip",
        required=True,
    ...
    print(f"Migrated multichoice-only columns: {'yes' if migrated else 'already present'}")
